...
const sortedByBirthYear = inventors.sort((inventor, nextInventor) => {
    return inventor.year - nextInventor.year;
});
...
isAdultPresent = devs.some((dev) => {
    let yearNow = Date().split(' ')[3]; // get current year
    return (yearNow - dev.year >= 18);
})
...
travelMethodCounts = travelMethods.reduce((acc, tMethod) => {
    if (acc[tMethod]) {
        acc[tMethod]++;
    } else {
        acc[tMethod] = 1;
